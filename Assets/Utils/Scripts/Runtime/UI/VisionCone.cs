...
using System.Linq;
using UnityEngine;
using Utils.Scripts.Runtime;
using Utils.Scripts.Runtime.ObjectPool;

namespace GameLogic.Utils {
    public class VisionCone : MonoSingleton<VisionCone> {
        public string[] tags; // 障碍物Tag

        [Range(0.6f, 4)]
        public float range = 3; // 视角距离

        [Range(5, 50)]
        public int samples = 12; // 取样点

        public float maxDegree = 20f, minDegree = -20f; // 视角上下限

        [Range(-10, 20)]
        public float sightDegree; // 视角动态旋转
